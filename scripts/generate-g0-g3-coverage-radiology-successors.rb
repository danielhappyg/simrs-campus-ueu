...
require_relative 'generate-g0-g3-coverage-tariff-successors'

# Append-only successor publisher for the bounded radiology tariff/source
# evidence. R3/R5 are immutable inputs; this module can change engineering
# evidence only and deliberately has no selector, owner, gate or deployment API.
module G0G3CoverageRadiologySuccessors
  ...
  GENERATOR_PATH = 'scripts/generate-g0-g3-coverage-radiology-successors.rb'
  SUPERSESSION_RELATIONSHIP = 'supersedes_without_rewriting_or_reinterpreting_predecessor'

  MAP_PREDECESSOR_PATH = 'docs/new-simrs-rebuild/G0_G3_COVERAGE_EVIDENCE_MAP_V2_2026-09-02_R3.json'
  MAP_PREDECESSOR_SHA256 = 'c59c15593c0db6f37ef144f95a7d04d22df60404ee7cf2e68562501cc07316f8'
  MAP_PREDECESSOR_ARTIFACT_ID = 'COVERAGE-ENGINEERING-EVIDENCE-MAP-V2-2026-09-02-R3'
  MAP_OUTPUT_PATH = 'docs/new-simrs-rebuild/G0_G3_COVERAGE_EVIDENCE_MAP_V2_2026-09-02_R4.json'
  MAP_ARTIFACT_ID = 'COVERAGE-ENGINEERING-EVIDENCE-MAP-V2-2026-09-02-R4'

  LEDGER_PREDECESSOR_PATH = 'docs/new-simrs-rebuild/G0_G3_COVERAGE_LEDGER_V2_2026-09-02_R5.json'
  LEDGER_PREDECESSOR_SHA256 = '1cef27463e0d5f7c887e68d6031671e13b3b7bbf980bd7ed1c01db6e73ec09f2'
  LEDGER_PREDECESSOR_ARTIFACT_ID = 'G0-G3-COVERAGE-LEDGER-V2-2026-09-02-R5'
  LEDGER_OUTPUT_PATH = 'docs/new-simrs-rebuild/G0_G3_COVERAGE_LEDGER_V2_2026-09-02_R6.json'
  LEDGER_ARTIFACT_ID = 'G0-G3-COVERAGE-LEDGER-V2-2026-09-02-R6'
  ...
  EVIDENCE_PATHS = %w[
    ...
    docs/operations/T1_LOCAL_RADIOLOGY_TARIFF_SOURCE_EVIDENCE_2026-09-02.md
    ...
    tests/Feature/Finance/FinanceRadiologyTariffHttpWorkflowTest.php
  ...
  def write_map!(root: repository_root, output: MAP_OUTPUT_PATH)
    ...
    receipt('generated_radiology_engineering_evidence_map_successor', root_path, output, bytes)
  end
  ...
  def write_ledger!(root: repository_root, output: LEDGER_OUTPUT_PATH)
    ...
    receipt('generated_radiology_coverage_ledger_successor', root_path, output, bytes)
  end
  ...
  def check_map!(root: repository_root, output: MAP_OUTPUT_PATH)
    ...
    raise Error, 'stale radiology engineering-evidence map successor' unless actual == serialized_map(root: root_path)
    true
  end
  ...
  def check_ledger!(root: repository_root, output: LEDGER_OUTPUT_PATH)
    ...
    raise Error, 'stale radiology coverage-ledger successor' unless actual == serialized_ledger(root: root_path)
    true
  end
  ...
  def validate_map!(document, root: repository_root, predecessor: nil)
    ...
    unless Core.canonical_json(document) == Core.canonical_json(expected)
      raise Error, '$.engineering_evidence_map_successor: differs from closed radiology projection'
    end
    true
  rescue Core::Error, KeyError => e
    raise Error, e.message
  ...
  def validate_ledger!(document, root: repository_root, predecessor: nil, map: nil, map_sha256: nil)
    ...
    unless Core.canonical_json(document) == Core.canonical_json(expected)
      raise Error, '$.coverage_ledger_successor: differs from closed observation-only radiology projection'
    end
    true
  rescue Core::Error, KeyError => e
    raise Error, e.message
...
if $PROGRAM_NAME == __FILE__
  ...
  parser = OptionParser.new do |opts|
    opts.banner = 'Usage: ruby scripts/generate-g0-g3-coverage-radiology-successors.rb --kind map|ledger [--output PATH] [--check]'
  ...
  begin
    parser.parse!(ARGV)
    raise G0G3CoverageRadiologySuccessors::UsageError, 'unexpected positional arguments' unless ARGV.empty?
    raise G0G3CoverageRadiologySuccessors::UsageError, '--kind is required' unless options[:kind]
    root = File.expand_path('..', __dir__)
    default = options[:kind] == 'map' ? G0G3CoverageRadiologySuccessors::MAP_OUTPUT_PATH : G0G3CoverageRadiologySuccessors::LEDGER_OUTPUT_PATH
    output = options[:output] || default
    result = if options[:check]
               options[:kind] == 'map' ? G0G3CoverageRadiologySuccessors.check_map!(root: root, output: output) : G0G3CoverageRadiologySuccessors.check_ledger!(root: root, output: output)
             elsif options[:kind] == 'map'
               G0G3CoverageRadiologySuccessors.write_map!(root: root, output: output)
             else
               G0G3CoverageRadiologySuccessors.write_ledger!(root: root, output: output)
             end
    puts G0ProportionalGovernanceV2.canonical_json(result.is_a?(Hash) ? result : { 'status' => 'ok' })
  rescue OptionParser::ParseError, G0G3CoverageRadiologySuccessors::UsageError => e
    warn e.message
    warn parser.banner
    exit 2
  rescue G0G3CoverageRadiologySuccessors::Error => e
    warn "coverage radiology successor generation failed: #{e.message}"
    exit 1
  end
end
