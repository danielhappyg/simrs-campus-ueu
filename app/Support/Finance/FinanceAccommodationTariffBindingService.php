...
final class FinanceAccommodationTariffBindingService
{
    private const DEFINITION = 'INPATIENT_ACCOMMODATION_OCCUPANCY_DAY_TARIFF_SOURCE_V1';

    public function __construct(
        private readonly FinanceAccommodationTariffActorPolicy $policy,
        private readonly FinanceAccommodationTariffContentDigest $digests,
    ...
    public function create(
        User $actor,
        string $inpatientBedVersionPublicId,
        string $careSetting,
    ...
    ): FinanceAccommodationTariffMutationResult {
        return $this->audited($actor, FinanceAccommodationTariffOperation::CREATE, null, $careSetting, function () use (
            $actor, $inpatientBedVersionPublicId, $careSetting, $tariffItemPublicId,
            ...
            $this->ulid($inpatientBedVersionPublicId);
            $this->ulid($tariffItemPublicId);
            $this->correlation($correlationId);

            $payload = compact(
                'inpatientBedVersionPublicId', 'careSetting', 'tariffItemPublicId',
                'effectiveFrom', 'reason',
            );

            return $this->execute(
                $actor,
                FinanceAccommodationTariffOperation::CREATE,
                $idempotencyKey,
                $payload,
                $correlationId,
                function () use ($actor, $inpatientBedVersionPublicId, $careSetting, $tariffItemPublicId, $effectiveFrom, $reason, $correlationId): FinanceAccommodationTariffBinding {
                    [$bed, $bedVersion] = $this->retainedBedVersion($inpatientBedVersionPublicId);
                    ...
                    $binding = new FinanceAccommodationTariffBinding([
                        'inpatient_bed_id' => $bed->id,
                        'inpatient_bed_version_id' => $bedVersion->id,
                        ...
                        'ward_public_id' => $bed->ward->public_id,
                        'ward_code' => $bed->ward->code,
                        'bed_public_id' => $bed->public_id,
                        'bed_code' => $bed->code,
                        'service_class' => $bedVersion->service_class,
                        'care_setting' => $careSetting,
                        'pricing_unit' => 'OCCUPANCY_DAY',
                        'state' => FinanceAccommodationTariffBinding::ACTIVE,
                        'version' => 1,
                        'latest_effective_from' => $effectiveFrom,
                        'current_content_digest' => str_repeat('0', 64),
                    ]);
                    $attributes = $this->versionAttributes($binding, $bed, $tariffItem, $actor, FinanceAccommodationTariffBinding::ACTIVE, $effectiveFrom, $reason, null, $correlationId);
                    ...
                    FinanceAccommodationTariffBindingVersion::query()->create($attributes);
    ...
    public function appendVersion(
        ...
        return $this->audited($actor, FinanceAccommodationTariffOperation::APPEND_VERSION, $bindingPublicId, null, function () use (
            ...
            return $this->execute(
                $actor,
                FinanceAccommodationTariffOperation::APPEND_VERSION,
                $idempotencyKey,
                $payload,
                $correlationId,
                function () use ($actor, $bindingPublicId, $tariffItemPublicId, $effectiveFrom, $expectedVersion, $expectedDigest, $reason, $correlationId): FinanceAccommodationTariffBinding {
                    $binding = FinanceAccommodationTariffBinding::query()->where('public_id', $bindingPublicId)->lockForUpdate()->firstOrFail();
                    $this->assertMutable($binding, $expectedVersion, $expectedDigest);
                    $this->assertNextDate($binding, $effectiveFrom);
                    $master = InpatientBed::query()->with('ward')->whereKey($binding->inpatient_bed_id)->firstOrFail();
                    $this->assertBindingBed($binding);
                    $tariffItem = FinanceTariffItem::query()->where('public_id', $tariffItemPublicId)->lockForUpdate()->firstOrFail();
                    $this->assertTariffContext($tariffItem, $binding->care_setting, $effectiveFrom);
                    $previous = $binding->current_content_digest;
                    $binding->state = FinanceAccommodationTariffBinding::ACTIVE;
                    $binding->version++;
                    $binding->latest_effective_from = Carbon::parse($effectiveFrom);
                    $attributes = $this->versionAttributes($binding, $master, $tariffItem, $actor, FinanceAccommodationTariffBinding::ACTIVE, $effectiveFrom, $reason, $previous, $correlationId);
                    $binding->current_content_digest = $this->digests->binding($attributes);
                    $binding->save();
                    $attributes['binding_id'] = $binding->id;
                    $attributes['content_digest'] = $binding->current_content_digest;
    ...
    public function retire(
        ...
        return $this->audited($actor, FinanceAccommodationTariffOperation::RETIRE, $bindingPublicId, null, function () use (
            ...
            return $this->execute(
                $actor,
                FinanceAccommodationTariffOperation::RETIRE,
                $idempotencyKey,
                $payload,
                $correlationId,
                function () use ($actor, $bindingPublicId, $effectiveFrom, $expectedVersion, $expectedDigest, $reason, $correlationId): FinanceAccommodationTariffBinding {
                    $binding = FinanceAccommodationTariffBinding::query()->where('public_id', $bindingPublicId)->lockForUpdate()->firstOrFail();
                    $this->assertMutable($binding, $expectedVersion, $expectedDigest);
                    $this->assertNextDate($binding, $effectiveFrom);
                    $master = InpatientBed::query()->with('ward')->whereKey($binding->inpatient_bed_id)->firstOrFail();
                    $this->assertBindingBed($binding);
                    $latest = FinanceAccommodationTariffBindingVersion::query()->where('binding_id', $binding->id)->orderByDesc('version')->firstOrFail();
                    $tariffItem = FinanceTariffItem::query()->whereKey($latest->tariff_item_id)->firstOrFail();
                    $previous = $binding->current_content_digest;
                    $binding->state = FinanceAccommodationTariffBinding::RETIRED;
                    $binding->version++;
                    $binding->latest_effective_from = Carbon::parse($effectiveFrom);
                    $attributes = $this->versionAttributes($binding, $master, $tariffItem, $actor, FinanceAccommodationTariffBinding::RETIRED, $effectiveFrom, $reason, $previous, $correlationId);
                    $binding->current_content_digest = $this->digests->binding($attributes);
                    $binding->save();
                    $attributes['binding_id'] = $binding->id;
                    $attributes['content_digest'] = $binding->current_content_digest;
     ...
     * @param  callable(): FinanceAccommodationTariffBinding  $write
     */
    private function execute(User $actor, string $operation, string $key, array $payload, ?string $correlationId, callable $write): FinanceAccommodationTariffMutationResult
    {
        ...
        try {
            return DB::transaction(function () use ($actor, $operation, $key, $payloadDigest, $correlationId, $write): FinanceAccommodationTariffMutationResult {
                return FinanceAccommodationTariffMutationScope::run(function () use ($actor, $operation, $key, $payloadDigest, $correlationId, $write): FinanceAccommodationTariffMutationResult {
                    ...
                    $binding = $write();
                    $version = FinanceAccommodationTariffBindingVersion::query()
                    ...
                    FinanceAccommodationTariffOperationReceipt::query()->create([
                        'actor_user_id' => $actor->id,
                        'operation' => $operation,
                        'idempotency_key' => $key,
                        'payload_digest' => $payloadDigest,
                        'result_type' => FinanceAccommodationTariffOperationReceipt::RESULT_BINDING,
                    ...
                    return new FinanceAccommodationTariffMutationResult($binding, false);
                });
            }, 3);
            ...
            throw new FinanceTariffDenied('concurrent_state_conflict', 'Operasi bersamaan telah mengubah pemetaan tarif akomodasi.');
    ...
    private function replay(User $actor, string $operation, string $key, string $payloadDigest): ?FinanceAccommodationTariffMutationResult
    {
        $receipt = FinanceAccommodationTariffOperationReceipt::query()
            ->where('actor_user_id', $actor->id)->where('operation', $operation)
            ->where('idempotency_key', $key)->first();
        if (! $receipt instanceof FinanceAccommodationTariffOperationReceipt) {
            return null;
        }
        if (! hash_equals($receipt->payload_digest, $payloadDigest)
            || $receipt->result_type !== FinanceAccommodationTariffOperationReceipt::RESULT_BINDING) {
            throw new FinanceTariffDenied('idempotency_key_conflict', 'Kunci idempotensi sudah digunakan untuk muatan lain.');
        }
        $binding = FinanceAccommodationTariffBinding::query()->where('public_id', $receipt->result_public_id)->first();
        $version = $binding instanceof FinanceAccommodationTariffBinding
            ? FinanceAccommodationTariffBindingVersion::query()->where('binding_id', $binding->id)
                ->where('version', $receipt->result_version)->first()
            : null;
        if (! $binding instanceof FinanceAccommodationTariffBinding || ! $version instanceof FinanceAccommodationTariffBindingVersion
            || ! hash_equals($version->content_digest, $this->digests->bindingVersion($binding, $version))) {
            throw new FinanceTariffDenied('receipt_corrupt', 'Bukti operasi pemetaan tarif akomodasi tidak dapat direkonsiliasi.');
        }
        ...
        if (! hash_equals($receipt->result_digest, $computed)) {
            throw new FinanceTariffDenied('receipt_corrupt', 'Digest bukti operasi pemetaan tarif akomodasi tidak valid.');
        }

        return new FinanceAccommodationTariffMutationResult($this->retainedView($binding, $version), true);
    }

    private function retainedView(FinanceAccommodationTariffBinding $binding, FinanceAccommodationTariffBindingVersion $version): FinanceAccommodationTariffBinding
    {
        $view = clone $binding;
    ...
    /** @return array{InpatientBed,InpatientBedVersion} */
    private function retainedBedVersion(string $publicId): array
    {
        $version = InpatientBedVersion::query()->where('public_id', $publicId)->firstOrFail();
        $master = InpatientBed::query()->with('ward')->whereKey($version->bed_id)->firstOrFail();
        $computed = $this->digests->inpatientBedVersion($master->code, $version->display_name, $version->room_label, $version->service_class, $version->state, $version->version);
        if (! hash_equals($version->after_digest, $computed) || $version->version < 1) {
            throw new FinanceTariffDenied('master_version_mismatch', 'Versi master akomodasi tidak dapat direkonsiliasi.');
        }

        return [$master, $version];
    }

    private function assertBindingBed(FinanceAccommodationTariffBinding $binding): void
    {
        [$master, $version] = $this->retainedBedVersion($binding->inpatient_bed_version_public_id);
        if ($master->id !== $binding->inpatient_bed_id || $version->id !== $binding->inpatient_bed_version_id
            || $version->version !== $binding->inpatient_bed_version
            || ! hash_equals($version->after_digest, $binding->inpatient_bed_content_digest)
            || $master->public_id !== $binding->bed_public_id
            || $master->ward->public_id !== $binding->ward_public_id
            || $master->ward->code !== $binding->ward_code
            || $master->code !== $binding->bed_code) {
            throw new FinanceTariffDenied('master_version_mismatch', 'Ikatan master akomodasi telah berubah.');
        }
    }
    ...
    private function assertTariffContext(FinanceTariffItem $item, string $careSetting, string $effectiveFrom): FinanceTariffItemVersion
    {
        ...
        if ($version->service_domain !== 'ACCOMMODATION' || $version->care_setting !== $careSetting) {
            throw new FinanceTariffDenied('source_binding_invalid', 'Item tarif harus berdomain ACCOMMODATION dan memiliki care setting INPATIENT.');
        }
    ...
    private function versionAttributes(FinanceAccommodationTariffBinding $binding, InpatientBed $master, FinanceTariffItem $item, User $actor, string $state, string $effectiveFrom, string $reason, ?string $previousDigest, ?string $correlationId): array
    {
        return [
            'actor_user_id' => $actor->id,
            'version' => $binding->version,
            'bed_public_id' => $master->public_id,
    ...
    private function assertMutable(FinanceAccommodationTariffBinding $binding, int $expectedVersion, string $expectedDigest): void
    {
        if ($binding->state === FinanceAccommodationTariffBinding::RETIRED) {
    ...
    private function assertNextDate(FinanceAccommodationTariffBinding $binding, string $effectiveFrom): void
    {
    ...
    private function careSetting(string $careSetting): string
    {
        ...
        if ($careSetting !== 'INPATIENT') {
            throw new FinanceTariffDenied('validation_failed', 'Pemetaan akomodasi hanya berlaku untuk care setting INPATIENT.');
        }
    ...
    private function recordSuccess(User $actor, string $operation, FinanceAccommodationTariffBinding $binding, bool $replayed, ?string $correlationId): void
    {
        if ($this->audit->record(
            ...
            metadata: [
                'operation' => $operation,
                'entity_type' => 'ACCOMMODATION_TARIFF_BINDING',
                'state' => $binding->state,
                'version' => $binding->version,
            ...
            throw new FinanceTariffAuditUnavailable('Perubahan pemetaan tarif akomodasi dibatalkan karena audit wajib tidak tersedia.');
        }
    }

    /** @param callable(): FinanceAccommodationTariffMutationResult $callback */
    private function audited(User $actor, string $operation, ?string $resource, ?string $careSetting, callable $callback): FinanceAccommodationTariffMutationResult
    {
        try {
            return $callback();
    ...
    private function recordDenial(User $actor, string $operation, ?string $resource, string $reason, ?string $careSetting): void
    {
        ...
        $metadata = ['operation' => $operation, 'entity_type' => 'ACCOMMODATION_TARIFF_BINDING'];
        ...
        if ($this->audit->record(
            ...
            throw new FinanceTariffAuditUnavailable('Penolakan pemetaan tarif akomodasi tidak dapat diaudit.');
        }
    }
