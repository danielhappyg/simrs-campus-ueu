...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        foreach (self::BASE_ROSTER + self::ADDED_ROSTER as $email => $rosterKey) {
            DB::table(SchemaQualifier::table('users'))
                ->where('email', $email)
                ->update(['teaching_access_roster_key' => $rosterKey]);
        }

        $this->addPostgresMappingConstraint(self::BASE_ROSTER + self::ADDED_ROSTER);
    }
    ...
    public function down(): void
    {
        ...
        if ($addedAccountIds->isNotEmpty()
            ...
            throw new RuntimeException(
                'Teaching-role roster rollback refused: preserve the added identities and their access-window evidence.',
            );
        }

        $this->dropPostgresMappingConstraint();
        $this->addPostgresMappingConstraint(self::BASE_ROSTER);
    }
