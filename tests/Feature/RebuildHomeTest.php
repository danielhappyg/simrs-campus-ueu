...
class RebuildHomeTest extends TestCase
{
    ...
    public function test_home_returns_only_todays_active_synthetic_totals_by_care_setting(): void
    {
        $user = User::factory()->create();
        ...
        $this->actingAs($user)
            ...
            ->assertInertia(fn (Assert $page) => $page
                ->where('encounters.available', true)
                ->where('encounters.totals.rawat_jalan', 2)
                ->where('encounters.totals.igd', 1)
                ->where('encounters.totals.rawat_inap', 1)
    ...
    public function test_rmik_action_is_hidden_without_rmik_review_and_shown_for_rmik_role(): void
    {
        ...
        $this->actingAs($rmik)
            ...
            ->assertInertia(fn (Assert $page) => $page
                ...
                ->where('actions.7.href', route('manajemen-data.bangsal.index')));
    }
    ...
    public function test_home_exposes_explicit_encounter_unavailable_state_on_read_failure(): void
    {
        ...
        $this->actingAs($user)
            ...
            ->assertOk()
            ->assertInertia(fn (Assert $page) => $page
                ->where('encounters.available', false)
                ->where('encounters.totals.rawat_jalan', null)
                ->where('encounters.totals.igd', null)
                ->where('encounters.totals.rawat_inap', null)
                ->where('encounters.read_error', 'Ringkasan kunjungan belum dapat dimuat. Silakan coba lagi.'));
    }
