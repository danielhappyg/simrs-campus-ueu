...
require_relative '../../scripts/rehearse-local-laboratory-tariff-source-portability'

class LocalLaboratoryTariffSourcePortabilityHarnessContractTest < Minitest::Test
  Harness = LocalLaboratoryTariffSourcePortabilityRehearsal
  ...
  def test_ruby_and_embedded_php_are_syntactically_valid
    ...
    worker = Tempfile.new(['laboratory-tariff-source-portability-contract-', '.php'])
    worker.write(@worker)
    worker.flush
  ...
  def test_closed_twenty_two_scenario_catalogue_is_bounded_and_explicit
    assert_equal %w[
      fresh-migration empty-down-reapply exact-role-boundary three-care-setting-bindings
      original-verified-result-service-date-resolution future-half-open-terminal-retirement
      upstream-result-chain-refusal order-specimen-draft-cancellation-no-charge
      one-original-verified-result-one-typed-source verified-amendment-acknowledgement-no-extra-charge
      laboratory-only-bill-snapshot
      partial-sync-unresolved-issue-refusal later-gap-resolution-new-version
      replay-key-conflict-retroactive-stale-refusal same-verified-result-import-concurrency
      competing-binding-writer-concurrency verification-synchronization-cutoff-concurrency
      application-sql-guard-refusal database-append-only-head-typed-source-refusal
  ...
  def test_exact_engines_and_current_source_sha_bindings_are_closed
    ...
    assert_equal 'database/migrations/2026_09_02_000400_create_laboratory_verified_result_tariff_source.php', Harness::MIGRATION_PATH
    assert_equal 'SIMRS_LOCAL_LABORATORY_TARIFF_SOURCE_PORTABILITY', Harness::EVIDENCE_KIND
    assert_equal 'READY_NOT_RUN', Harness::EXECUTION_STATE
    binding = harness.current_laboratory_tariff_source_bindings
    ...
    %w[
      ...
      scenario_catalog_sha256 runtime_grant_catalog_sha256 sqlite_gate_catalog_sha256
  ...
  def test_real_worker_uses_governed_services_and_typed_one_to_one_source
    %w[
      FinanceTariffMasterService FinanceLaboratoryTariffBindingService
      FinanceLaboratorySourceAdapter LaboratoryWorkflowService LaboratoryResultVersion
      FinanceSourceCoordinator FinanceBillService LaboratoryMasterService
    ].each { |name| assert_includes @worker, name }
    assert_includes @worker, 'one typed charge per original verified result'
    assert_includes @worker, 'original immutable trigger'
    assert_includes @worker, 'base_verified_version_id === null'
    assert_includes @worker, "'source_domain' => $charges->sole()->source_domain"
    assert_includes @worker, "'unit_amount' => $source->unit_amount"
    assert_includes @worker, 'verifyRetained'
    assert_includes @worker, "'unresolved_count'"
  end

  def test_three_independent_process_races_observe_real_database_waits_and_visibility
    ...
    assert_includes @worker, "'lock_order_trace' => ['finance_laboratory_tariff_bindings']"
    ...
    assert_includes @source, 'run_visibility_race!'
    assert_includes @source, "outcomes == %w[MATERIALIZED VERIFIED]"
    assert_includes @source, "'visibility_rule' => 'committed_original_verified_result_only'"
  end
  ...
  def test_sqlite_gate_is_closed_hashed_and_required_before_exact_engine_setup
    assert_equal Harness::FEATURE_SCENARIO_TESTS.values.uniq, Harness::SQLITE_FEATURE_TESTS
    assert_includes @source, 'def run_sqlite_gate!'
    ...
    assert_includes @source, "'sqlite_gate' => sqlite_gate"
    assert_includes @source, "'evidence_sha256' => Digest::SHA256.file(evidence_path).hexdigest"
  end
  ...
  def test_evidence_is_create_only_sanitized_strictly_cleaned_and_keeps_gates_open
    ...
    assert_includes @source, "assert_unchanged_binding!('Laboratory tariff/source execution bindings'"
