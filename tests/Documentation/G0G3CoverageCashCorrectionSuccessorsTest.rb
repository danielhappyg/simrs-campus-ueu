...
require_relative '../../scripts/generate-g0-g3-coverage-cash-correction-successors'

class G0G3CoverageCashCorrectionSuccessorsTest < Minitest::Test
  ROOT = File.expand_path('../..', __dir__)
  Generator = G0G3CoverageCashCorrectionSuccessors
  Core = G0ProportionalGovernanceV2

  MAP_SHA256 = '4bdfb6bf3f528b0d09b048049e2b047a678e8cb5711133929b6b494f15ae0415'
  LEDGER_SHA256 = '40ed0a478be4c7e9077cbbe3e46609ff900cf9e8e075c93a52aad0b6a3a50bb8'
  EXACT_EVIDENCE = {
  ...
  def setup
    ...
    @tmpdir = Pathname.new(Dir.mktmpdir('.g0-g3-cash-correction-successors-', ROOT)).realpath
  ...
  def test_r8_r10_remain_exact_immutable_historical_successors
    assert_equal 'COVERAGE-ENGINEERING-EVIDENCE-MAP-V2-2026-09-02-R7', Generator::MAP_PREDECESSOR_ARTIFACT_ID
    assert_equal 'COVERAGE-ENGINEERING-EVIDENCE-MAP-V2-2026-09-02-R8', Generator::MAP_ARTIFACT_ID
    assert_equal 'G0-G3-COVERAGE-LEDGER-V2-2026-09-02-R9', Generator::LEDGER_PREDECESSOR_ARTIFACT_ID
    assert_equal 'G0-G3-COVERAGE-LEDGER-V2-2026-09-02-R10', Generator::LEDGER_ARTIFACT_ID
  ...
  def test_ledger_preserves_governance_authority_gates_and_every_non_target_row
    %w[
  ...
  def test_exact_correction_evidence_hash_modes_sources_scenarios_and_open_claims
    ...
    [Generator::POSTGRES_ARTIFACT_PATH, Generator::MYSQL_ARTIFACT_PATH].each do |relative|
      ...
      assert_equal 'SIMRS_LOCAL_APPEND_ONLY_CASH_SETTLEMENT_CORRECTION_PORTABILITY', artifact.fetch('kind')
      ...
      recorded_files = artifact.dig('source_bindings', 'files')
      assert recorded_files.keys.all? { |path| File.file?(@root.join(path)) }
      assert_equal artifact.dig('source_bindings', 'aggregate_sha256'),
                   artifact.dig('source_bindings', 'application_source_sha256')
    end
  end

  def test_create_only_refuses_overwrite_and_predecessor_hash_drift
  ...
  def test_readme_retains_r8_r10_as_immutable_predecessors_without_closing_governance
    ...
    assert_match(/R10.*immutable/m, readme)
