...
- a reusable unsaved-change guard on the versioned nursing, medical, and closure authoring forms, with a visible local-change state, explicit stay/save-draft-then-leave/discard choices, and an in-context failure/retry state that never abandons the encounter;
