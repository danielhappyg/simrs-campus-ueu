...
A draft save keeps the guard open in a disabled `Menyimpan draf…` state until the request resolves. Validation, HTTP, network, or cancelled-request failure leaves the user on the same encounter form, announces `Draf belum tersimpan` without copying field content, restores `Simpan draf lalu keluar` for retry, and does not continue navigation. In-session history entries carry only an integer position—never form content—and Back/Forward replays the exact held target after successful save or explicit discard.

Each versioned nursing, medical-assessment, and encounter-closure submission—including save-and-leave—carries a non-clinical recovery header. Authentication or CSRF expiry returns a generic no-store `401/419` response rather than an Inertia login-page replacement. The active form or departure dialog announces `Sesi masuk perlu dipulihkan`, keeps the delta only in the original tab's memory, and opens the normal login route in a new tab with `noopener noreferrer`. Returning to the original tab and retrying sends the form through the same authoritative endpoint, so current account, assignment, session, encounter, workflow, request key, and source version are revalidated before a version is accepted or any held navigation is replayed. Automatic reauthentication, cross-tab content transfer, cross-encounter recovery, and browser-persisted clinical drafts are not claimed. Live recovery rehearsal remains open before faculty pilot.
