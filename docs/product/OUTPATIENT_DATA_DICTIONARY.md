...
| `source_type`, `source_id`, `source_version_id` | values | yes | Exact clinician-authored diagnosis/procedure evidence. |
| `classification_system` | URI/string | yes | e.g. ICD-10 or configured procedure classification. |
...
| `status` | enum | yes | `DRAFT`, `SUBMITTED`, `APPROVED`, `RETURNED`. |
...
| Coding assignment has no source diagnosis/version. | Whether the selected diagnosis is clinically correct. |
