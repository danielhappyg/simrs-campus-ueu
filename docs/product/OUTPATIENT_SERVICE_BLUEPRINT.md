...
| 9. RMIK review | RMIK learner assembles the record, checks required documentation/sign-off, assigns diagnosis/procedure codes from source statements, and records findings. | Run deterministic completeness checks, link codes to source entries, keep terminology version, route correction requests to the author. | Completeness review, coding assignments, correction requests. | No blocking findings and RMIK review approved. |
