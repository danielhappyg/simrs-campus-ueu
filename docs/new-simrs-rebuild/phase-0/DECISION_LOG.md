...
| DEC-014 | 2026-08-21 | Accepted | **SAHABAT Data Pasien desk density is the minimum bar** for Pendaftaran (and later care desks that replace SAHABAT surfaces): three-column layout; top action strip; left Data Pribadi through wilayah/contact/notes; center PJ + poli→dokter→jadwal + cara masuk/bayar; right antrian/print stubs + Simpan. Visual oracle = `docs/new-simrs-rebuild/_evidence/sahabat-page-1.png` / `sahabat-page-2.png` (from user `SIMRS SAHABAT.pdf`) cross-checked with live capture `CAP-REG-005` / classic `CAP-REG-003`. Thin ~7-field MVP forms are non-compliant. Production BPJS/SEP/SatuSehat/FR/FP send remain forbidden; stubs must be honest. Classic 224-control footprint is discovery backlog, not automatic ship-all. | Product owner | User approval 2026-08-21 (“follow the minimum requirement in the sahabat system” + assess screenshots/folder). See `CURRENT_BASELINE.md` §4, `PENDAFTARAN_SAHABAT_FIELD_MAP.md`. |
| DEC-015 | 2026-08-23 | Accepted | Every interactive teaching screen, including authentication and authenticated application screens, must retain a restrained, permanent and non-dismissible **`SIMULASI — DATA SINTETIS`** indicator in the shared application shell. The indicator is a safety boundary, not the product's primary visual identity: keep it compact and consistent with UEU tokens. It supplements, and does not replace, backend synthetic-only enforcement. Real patient data and production BPJS/VClaim/SATUSEHAT connections remain forbidden without a new explicit decision. | Product owner | User approval 2026-08-23. Reconciles prior contradictory banner wording in `phase-1/UI_DIRECTION.md` and teaching operations docs. Does not weaken DEC-013: the failed Antrean/work-queue MVP remains an anti-reference. Reversal requires a new DEC plus an equivalent visible simulation boundary. |
| DEC-016 | 2026-08-23 | **Proposed** | For the bounded outpatient lab teaching slice: `ACTIVE` lab orders block RM closure; `CLOSED` encounters reject late results; only one immutable `FINAL` result is accepted and completes an active order. Preliminary results, amendment, reopen and cancellation remain unbuilt pending an approved workflow. | Clinical/Laboratory owner + RMIK Department | **Proposed NEW teaching safety; not observed/validated SIMRS Sahabat parity.** Product owner approved the bounded work, but Clinical/Laboratory and RMIK acceptance remains unresolved. See `phase-1/OUTPATIENT_ORDER_RESULT_CLOSURE_CONTRACT.md`. Do not mark PAR-CLN-006 or PAR-RMIK-001 Accepted on this basis. |
...
- Permanent visible simulation boundary: DEC-015 + `docs/new-simrs-rebuild/phase-1/UI_DIRECTION.md`
- Proposed outpatient order/result/closure safety contract: DEC-016 + `docs/new-simrs-rebuild/phase-1/OUTPATIENT_ORDER_RESULT_CLOSURE_CONTRACT.md`
