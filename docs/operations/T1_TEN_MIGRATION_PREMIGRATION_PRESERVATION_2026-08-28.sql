...
-- for the exact ten-migration 2026-08-28 candidate.
...
BEGIN
    ...
    IF to_regclass('laravel.inpatient_bed_claim_mutexes') IS NOT NULL
       OR to_regclass('laravel.inpatient_bed_claim_mutexes_pkey') IS NOT NULL
       OR to_regclass('laravel.encounters_care_bed_status_idx') IS NOT NULL THEN
        RAISE EXCEPTION 'PRE_MIGRATION_NO_GO: inpatient bed mutex candidate objects already exist';
    END IF;
    ...
    -- to gain ten rows. cache/cache_locks/sessions and queue-runtime tables are
    ...
    -- rewritten by the ten migrations; every other preexisting column is hashed.
    FOR table_row IN
...
) AS ten_migration_premigration_preservation
FROM context;
