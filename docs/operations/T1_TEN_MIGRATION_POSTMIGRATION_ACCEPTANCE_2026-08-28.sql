-- SIMRS Campus UEU: fail-closed acceptance for the exact ten-migration candidate.
...
-- T1_TEN_MIGRATION_PREMIGRATION_PRESERVATION_2026-08-28.sql. Undefined psql
...
BEGIN
    ...
    IF observed_count <> 42 THEN
        RAISE EXCEPTION 'POST_MIGRATION_NO_GO: expected 42 laravel base tables, found %', observed_count;
    END IF;

    -- Exact ordered ledger: the 31-row reviewed predecessor plus ten rows in
    ...
    WITH expected(id, migration, batch) AS (
        VALUES
            ...
            (41, '2026_08_28_000100_create_inpatient_bed_claim_mutexes', 8)
    ), actual AS (
    ...
    IF contract_ok IS NOT TRUE THEN
        RAISE EXCEPTION 'POST_MIGRATION_NO_GO: exact 41-row migration ledger or batch-8 order drifted';
    END IF;
    ...
    WITH expected(table_name, column_names, formatted_types, not_nulls) AS (
        ...
        ('inpatient_bed_claim_mutexes',
         ARRAY['bed_code','created_at','updated_at']::text[],
         ARRAY['character varying(64)','timestamp(0) without time zone','timestamp(0) without time zone']::text[],
    ...
    -- Only the reviewed defaults may exist on the eleven new tables. Every serial
    ...
    WITH expected(table_name, column_name, default_expression) AS (
        ...
        WHERE schema_row.nspname = 'laravel'
          AND table_row.relname IN (
              ...
              'teaching_role_access_leases','inpatient_bed_claim_mutexes'
          )
    )
    SELECT count(*) = 11
       AND NOT EXISTS (
    ...
    -- Exact indexes for the eleven new tables plus seven reviewed additions on
    -- existing tables.
    WITH expected(index_name, table_name, columns, is_unique, is_primary, predicate) AS (
        VALUES
            ...
            ('inpatient_bed_claim_mutexes_pkey','inpatient_bed_claim_mutexes',ARRAY['bed_code']::text[],true,true,NULL),
            ('encounters_care_bed_status_idx','encounters',ARRAY['care_setting','bed_code','status']::text[],false,false,NULL)
    ), actual AS (
    ...
    -- Every candidate primary/unique constraint is bound to its exact name,
    -- type, validated/non-deferrable state, and already-reviewed backing index.
    -- Together with the exact index-column contract above and foreign-key
    -- contract below, this is a normalized semantic constraint definition.
    WITH expected(constraint_name, constraint_type, backing_index_name) AS (
        VALUES
            ('break_glass_requests_pkey','p','break_glass_requests_pkey'),
            ('bgr_public_id_uq','u','bgr_public_id_uq'),
            ('bgr_canonical_digest_uq','u','bgr_canonical_digest_uq'),
            ('break_glass_decisions_pkey','p','break_glass_decisions_pkey'),
            ('bgd_public_id_uq','u','bgd_public_id_uq'),
            ('bgd_request_uq','u','bgd_request_uq'),
            ('bgd_canonical_digest_uq','u','bgd_canonical_digest_uq'),
            ('break_glass_activations_pkey','p','break_glass_activations_pkey'),
            ('bga_public_id_uq','u','bga_public_id_uq'),
            ('bga_request_uq','u','bga_request_uq'),
            ('bga_decision_uq','u','bga_decision_uq'),
            ('bga_nonce_digest_uq','u','bga_nonce_digest_uq'),
            ('bga_canonical_digest_uq','u','bga_canonical_digest_uq'),
            ('break_glass_revocations_pkey','p','break_glass_revocations_pkey'),
            ('bgrv_public_id_uq','u','bgrv_public_id_uq'),
            ('bgrv_activation_uq','u','bgrv_activation_uq'),
            ('bgrv_canonical_digest_uq','u','bgrv_canonical_digest_uq'),
            ('break_glass_session_bindings_pkey','p','break_glass_session_bindings_pkey'),
            ('bgsb_public_id_uq','u','bgsb_public_id_uq'),
            ('bgsb_activation_uq','u','bgsb_activation_uq'),
            ('bgsb_session_hmac_uq','u','bgsb_session_hmac_uq'),
            ('bgsb_canonical_digest_uq','u','bgsb_canonical_digest_uq'),
            ('break_glass_subject_leases_pkey','p','break_glass_subject_leases_pkey'),
            ('bgsl_public_id_uq','u','bgsl_public_id_uq'),
            ('bgsl_subject_uq','u','bgsl_subject_uq'),
            ('bgsl_activation_uq','u','bgsl_activation_uq'),
            ('security_ledger_entries_pkey','p','security_ledger_entries_pkey'),
            ('sle_public_id_uq','u','sle_public_id_uq'),
            ('sle_semantic_key_uq','u','sle_semantic_key_uq'),
            ('sle_integrity_digest_uq','u','sle_integrity_digest_uq'),
            ('security_ledger_outboxes_pkey','p','security_ledger_outboxes_pkey'),
            ('slo_public_id_uq','u','slo_public_id_uq'),
            ('slo_entry_uq','u','slo_entry_uq'),
            ('daily_queue_counters_pkey','p','daily_queue_counters_pkey'),
            ('teaching_role_access_leases_pkey','p','teaching_role_access_leases_pkey'),
            ('tral_public_id_uq','u','tral_public_id_uq'),
            ('tral_credential_commitment_uq','u','tral_credential_commitment_uq'),
            ('tral_one_active_slot_uq','u','tral_one_active_slot_uq'),
            ('inpatient_bed_claim_mutexes_pkey','p','inpatient_bed_claim_mutexes_pkey')
    ), actual AS (
        SELECT constraint_row.conname::text AS constraint_name,
               constraint_row.contype::text AS constraint_type,
               index_row.relname::text AS backing_index_name,
               constraint_row.convalidated
                 AND NOT constraint_row.condeferrable
                 AND NOT constraint_row.condeferred AS healthy
    ...
    SELECT count(*) = 39
       AND NOT EXISTS (
           ...
           FULL OUTER JOIN actual
            ...
            AND actual.backing_index_name = expected.backing_index_name
            AND actual.healthy
    ...
    FROM actual;
    IF contract_ok IS NOT TRUE THEN
        RAISE EXCEPTION 'POST_MIGRATION_NO_GO: primary/unique constraint contract drifted';
    END IF;

    SELECT count(*) = 0 INTO contract_ok
    FROM pg_constraint constraint_row
    JOIN pg_class table_row ON table_row.oid = constraint_row.conrelid
    JOIN pg_namespace schema_row ON schema_row.oid = table_row.relnamespace
    WHERE schema_row.nspname = 'laravel'
      AND constraint_row.contype NOT IN ('p','u','f')
      AND table_row.relname IN (
          'break_glass_requests','break_glass_decisions','break_glass_activations',
          'break_glass_revocations','break_glass_session_bindings','break_glass_subject_leases',
          'security_ledger_entries','security_ledger_outboxes','daily_queue_counters',
    ...
    IF contract_ok IS NOT TRUE THEN
        RAISE EXCEPTION 'POST_MIGRATION_NO_GO: unexpected candidate CHECK/exclusion constraint is present';
    END IF;
    ...
    IF EXISTS (SELECT 1 FROM laravel.inpatient_bed_claim_mutexes) THEN
        RAISE EXCEPTION 'POST_MIGRATION_NO_GO: inpatient bed mutex migration unexpectedly created claim rows';
    END IF;
...
WITH api_roles(role_name) AS (
    ...
    SELECT
        ...
        (SELECT count(*) FROM laravel.inpatient_bed_claim_mutexes) AS inpatient_bed_claim_mutexes
)
SELECT jsonb_build_object(
    ...
    'migration_ledger', jsonb_build_object(
        ...
        'last_candidate', '2026_08_28_000100_create_inpatient_bed_claim_mutexes'
    ),
    'catalog_contracts', jsonb_build_object(
        'candidate_new_tables', 11,
...
) AS ten_migration_postmigration_acceptance
FROM counts
