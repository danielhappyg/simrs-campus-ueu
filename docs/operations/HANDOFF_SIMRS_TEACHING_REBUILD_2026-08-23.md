...
- A reusable current-manifest portability harness now passed fresh migration and the complete 451-test PHP suite on a harness-owned PostgreSQL 17.10 cluster/private Unix socket/`laravel` schema (450 passed, 1 skipped; 5,724 assertions) and an isolated exact MySQL 8.4.11/InnoDB server (444 passed, 7 skipped; 5,695 assertions). E2E-01/02/03/04/05/12/15/16 focused slices also passed on both engines against identical pre/post backend, migration, harness, workflow-catalogue, and pre-run manifest bindings. The ordinary manifest gate parses both records and rejects stale inner bindings; bootstrap mode cannot claim PASS. This is local PHP 8.5.7 portability evidence only; hosted execution, PHP 8.3 equivalence, contention/load, and owner acceptance remain open. See `docs/operations/T1_LOCAL_CURRENT_MANIFEST_PORTABILITY_EVIDENCE_2026-08-27.md`.
