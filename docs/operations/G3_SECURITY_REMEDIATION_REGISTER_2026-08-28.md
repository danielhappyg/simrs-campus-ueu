...
**LOCAL TECHNICAL REMEDIATION PASS / SEALED DIFFS: 0 REPORTABLE / SOURCE EDGE CONTROL CLOSED / HOSTED EDGE-CACHE PROOF DEFERRED / NOT PUSHED / NOT DEPLOYED / G3 NOT ACCEPTED**
...
- Deferred proof 1: source policy now avoids broad trust, but the exact hosted candidate must still prove Vercel sanitization, direct-origin exclusion, assigned-host behavior, private-schema framework-table resolution, and atomic shared rate-limit/cache state.
...
The [local trusted-edge and shared-maintenance validation](T1_LOCAL_TRUSTED_EDGE_SHARED_MAINTENANCE_VALIDATION_2026-08-28.md) then closed the source-level broad-header trust: direct entry trusts no forwarding headers, Vercel accepts only its documented overwritten client IP and protocol, Render accepts protocol only, and assigned hosts are exact-allowlisted. Disposable SQLite and exact PostgreSQL 17.10 rehearsals prove the maintenance marker crosses fresh PHP processes. The owned PostgreSQL proof additionally covers private `laravel` framework tables, atomic cache increments, database-lock exclusion/release, encrypted session continuity, prefix/key/schema divergence, cleanup-before-evidence, aggregate evidence scrubbing, failed-shutdown process termination, symlink-safe evidence paths, stable pre/post source/dependency binding, and installed-runtime/lockfile identity equality. Its contract passed `14` tests and `291` assertions. This remains local rather than hosted acceptance; edge sanitization, direct-origin exclusion, Supabase/PgBouncer resolution, hosted multi-instance atomicity and unavailable-store behavior remain action-time gates.

## Residual and action-time gates

The following remain open and prevent any G3 acceptance claim:

1. Complete the remaining hosted trusted-edge/direct-origin and Supabase shared-state proof on the exact candidate. Source header policy plus local SQLite/PostgreSQL independent-process maintenance, private-schema framework-table resolution, atomic counter, lock and encrypted-session behavior are closed; hosted sanitization, Supabase/PgBouncer resolution, multi-instance atomicity and unavailable-store behavior are not. The PostgreSQL bed-concurrency proof is also closed locally, and the old `1000-01-01` global sentinel is replaced by schema-qualified per-bed mutex rows in local commit `28ab1d8`.
...
- [Local trusted-edge and shared-maintenance validation](T1_LOCAL_TRUSTED_EDGE_SHARED_MAINTENANCE_VALIDATION_2026-08-28.md)
