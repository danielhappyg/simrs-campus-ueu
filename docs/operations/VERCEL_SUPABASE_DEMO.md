...
- Static files served through `api/assets.php` must be listed in `vercel.json`
  routes. The proven working path for binary brand marks is under
  `/build/brand/*` (same `/build/(.*)` route as Vite assets). `/favicon.svg`
  and `/robots.txt` are also routed. Paths that are not routed fall through
  to Laravel and will 404 even when the files exist in `public/`.
